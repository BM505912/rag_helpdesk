...
3.  **Seleccionar Medio de Contacto:** El sistema enviará el token de autenticación exclusivamente a los datos certificados del cliente en la base de Profuturo (correo electrónico o número celular previamente capturado y validado). No se solicitarán datos de contacto al cliente durante el proceso.
4.  **Validar Token:** El cliente recibirá el token en su medio certificado y deberá proporcionarlo para completar la autenticación. Este proceso garantiza mayor seguridad y transparencia, evitando la solicitud directa de datos de contacto.
...
El flujo del proceso va de la siguiente manera: `Módulo Cliente` > `Registra Clientes` > `Notas` > `APOVOL` > `Gerente`.
...
2.  **Consulta el Estatus del Trámite**: Si la cuenta bancaria del cliente está en proceso de certificación, el estatus del registro Apovol aparecerá como **'Registrado'**. Para conocer el estado actualizado y real del trámite, debes usar la función **Actualizar (Sincronizar)** en la aplicación. Si colocaste APOVOL en trámites de registro, podrás consultar el seguimiento en línea de este trámite hasta el momento del cargo.

3.  **Registro de EDR**: Ten en cuenta que el registro de EDR (Estudio Digital de Retiro) para clientes ahora se realiza **exclusivamente** a través de este módulo en Actividad PRO, sustituyendo a las antiguas Power Apps.
